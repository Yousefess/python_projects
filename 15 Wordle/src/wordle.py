...
class Wordle:
    ...
    def generate_word_frequency(self, file_path: str, word_len: int, limit: int):
        ...
        words_freq = list(filter(lambda w_freq: len(
            w_freq[0]) == word_len, words_freq))

        # Sort Data
        words_freq = sorted(
            words_freq, key=lambda w_freq: w_freq[1], reverse=True)

        # Limit Data
        words = words_freq[:limit]
    ...
    def run(self, ):
        ...
        while num_try:
            guess_word = input(
                f'Enter a {self.word_len} letter word (or q to exit): ')
            ...
            if len(guess_word) != 5:
                print(
                    f'Word must have {self.word_len} letters. You entered {len(guess_word)}!')
                continue
