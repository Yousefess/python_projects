...
base_currency = st.selectbox(
    'From Currency (Base):', currency_codes, index=currency_codes.index("USD"))
target_currency = st.selectbox(
    'To Currency (Target):', currency_codes, index=currency_codes.index("CAD"))
...
if amount > 0 and base_currency and target_currency:
    exchange_rate, time_last_updated = get_exchange_rate(API_KEY,
    ...
    if exchange_rate:
        ...
        st.success(
            f"✅ Exchange Rate: {exchange_rate:.4f} (Last updated: {time_ago})")
        col1, col2, col3 = st.columns(3)
        col1.metric(label="Base Currency", value=f"{amount} {base_currency}")
        # right arrow
        col2.markdown(
            "<h1 style='text-align: center; margin: 0;'>&#8594;</h1>", unsafe_allow_html=True)
        col3.metric(label="Target Currency",
                    value=f"{converted_amount:.2f} {target_currency}")
    else:
